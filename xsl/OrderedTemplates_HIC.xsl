...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="mods" exclude-result-prefixes="#all">
        <xsl:copy>
            ...
            <xsl:apply-templates select="note[@type='content'][@displayLabel='Boats']"/>
            <xsl:apply-templates select="note[@type='content'][@displayLabel='PT Class']"/>
            <xsl:apply-templates select="note[@type='content'][@displayLabel='PT Sub-Class']"/>
            <xsl:apply-templates select="note[@type='content'][@displayLabel='Plan No.']"/>
